...
- (void)viewDidLoad {
	...
	UITabBarItem* tabBarItem = [[UITabBarItem alloc] initWithTitle:@"上传" image:[UIImage imageNamed:@"upload.png"] tag:101];
	self.tabBarItem = tabBarItem;  //new add
    ...
    if (self) {
        ...
        [previewVc setDoneButtonClickBlockWithPreviewType:^(NSArray<UIImage *> *photos, NSArray *assets, BOOL isSelectOriginalPhoto) {
            ...
            [strongSelf dismissViewControllerAnimated:YES completion:^{
                ...
                {
                    NSLog(@"ImagePickController didFinishPickingPhotos...");
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    NSLog(@"album arr count: %lu ",_albumArr.count);
...
//        NSString *path = [[NSBundle mainBundle] pathForResource:@"Info" ofType:@"plist"];
        NSString *path = [[NSBundle mainBundle] pathForResource:@"LoongVBox-Info" ofType:@"plist"];
