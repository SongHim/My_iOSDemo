...
- (void)viewDidLoad
{
	...
	self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"同步" style:UIBarButtonItemStylePlain target:self action:@selector(showMessage)];
	self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"结束" style:UIBarButtonItemStylePlain target:self action:@selector(showENDMessage)];
	
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(MediaSyncEnd) name:@"MediaSyncEndNotification" object:nil];
...
{
	// 初始化对话框
	UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"提示" message:@"确认同步吗？" preferredStyle:UIAlertControllerStyleAlert];
	// 确定
...
- (void)showENDMessage
{
	[[NSNotificationCenter defaultCenter] postNotificationName:@"CompleteSynchronizationTask" object:self];
	NSString *mes = (NSString*)[[NSUserDefaults standardUserDefaults] objectForKey:MESSAGE];
	[self showError:mes];
...
-(void)uploadAsset
{
	/*NSString *identity = (NSString*)[[NSUserDefaults standardUserDefaults] objectForKey:IDENTITY];
	 BOOL identityisempty = [self isBlankString:identity];
	 if (identityisempty)
	 {
	 [self showError:@"请设置标识符！！！"];
	 return;
	 
	 NSString *Sockfd = (NSString*)[[NSUserDefaults standardUserDefaults] objectForKey:SOCKFD];
	 BOOL sockfdisempty = [self isBlankString:Sockfd];
	 if (sockfdisempty)
	 {
	 [self showError:@"请建立连接！！！"];
	 return;
	 }
	 }*/
	
	NSString *IP = (NSString*)[[NSUserDefaults standardUserDefaults] objectForKey:SERVER_NAME];
	NSString *PORT = (NSString*)[[NSUserDefaults standardUserDefaults] objectForKey:PORTID];
	int port = [PORT intValue];
	BOOL ipisempty = [self isBlankString:IP];
	BOOL portisempty = [self isBlankString:PORT];
	...
	NSLog(@"IP = %@", IP);
	NSLog(@"PORT = %@", PORT);
	int sock = Connected((char *)IP.UTF8String, port);
	NSLog(@"sockfdsssssssssss = %d", sock);
	if (sock <= 0)
	{
		[self showError:@"连接失败！！！"];
		return;
	...
	[self showError:@"同步中......"];
	[[NSNotificationCenter defaultCenter] postNotificationName:@"SyncMediaNotification" object:self];
}
...
- (void)dealloc
{
	[[NSNotificationCenter defaultCenter] removeObserver:self name:@"MediaSyncEndNotification" object:nil];
}
...
{
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    NSLog(@"show selected btn: %d ",tzImagePickerVc.showSelectBtn);
...
- (void)viewDidLayoutSubviews {
    ...
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    NSLog(@"layout show selected btn: %d ",tzImagePickerVc.showSelectBtn);
    tzImagePickerVc.showSelectBtn = NO;//niewei
    ...
    CGFloat toolBarHeight = 0;//[TZCommonTools tz_isIPhoneX] ? 50 + (83 - 49) : 50;//niewei
    if (self.navigationController.navigationBar.isTranslucent) {
        top = naviBarHeight;
        if (iOS7Later && !isStatusBarHidden)
...
- (void)didGetAllPhotos:(NSArray *)photos assets:(NSArray *)assets infoArr:(NSArray *)infoArr
{
    ...
    [tzImagePickerVc hideProgressHUD];
    NSLog(@"didGetAllPhotos hideProgressHUD...");
...
- (void)callDelegateMethodWithPhotos:(NSArray *)photos assets:(NSArray *)assets infoArr:(NSArray *)infoArr
{
    ...
    if (tzImagePickerVc.didFinishPickingPhotosHandle)
    {
        ...
        NSLog(@"PhotoPickerController didFinishPicking...");
    }
...
//    tzImagePickerVc.saveImageDataBlock(assets);
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.showSelectBtn = NO;//tzImagePickerVc.showSelectBtn;//niewei
    cell.allowPreview = NO;//tzImagePickerVc.allowPreview;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //tzImagePickerVc.allowPickingMultipleVideo = NO;
...
{
    NSLog(@"push photo preview controller....");
...
- (void)checkSelectedModels {
    ...
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    tzImagePickerVc.selectedModels = _models;
...
{
    NSLog(@"reloadPhotoArrayWithMediaType %@",mediaType);
